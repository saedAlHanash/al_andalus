...
class OffersProducts extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<ProductsCubit>()..getData(type: GetProductsType.offers),
      child: Builder(
        builder: (context) {
          return BlocBuilder<ProductsCubit, ProductsInitial>(
            builder: (context, state) {
              ...
              return Column(
                children: [
                  SeeAllHeader(
                    title: S.of(context).productsOnSale,
                    onTap: () {
                      Navigator.pushNamed(
                        context,
                        RouteName.products,
                        arguments: [context.read<ProductsCubit>(), S.of(context).productsOnSale],
                      );
                    },
                  ),
                  10.0.verticalSpace,
